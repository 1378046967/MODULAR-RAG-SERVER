...
class SparseRetriever:
    ...
    def _merge_results(
        ...
        for bm25_result, record in zip(bm25_results, records):
            ...
            # Handle case where record was not found (BM25 index and vector store
            # may be out of sync after partial re-ingestion or collection changes)
            if not record:
                logger.debug(
                    "No record found in vector store for chunk_id='%s'. Skipping.",
                    chunk_id,
                )
                continue
