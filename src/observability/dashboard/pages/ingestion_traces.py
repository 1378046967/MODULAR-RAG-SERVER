            ...
            if tab_defs:
                ...
                for tab, (label, key) in zip(tabs, tab_defs):
                    with tab:
                        ...
                        widget_key_prefix = f"ingestion_trace_{idx}_{key}"
                        if key == "load":
                            _render_load_stage(data, key_prefix=widget_key_prefix)
                        elif key == "split":
                            _render_split_stage(data, key_prefix=widget_key_prefix)
                        elif key == "transform":
                            _render_transform_stage(data, key_prefix=widget_key_prefix)
                        elif key == "embed":
                            _render_embed_stage(data, key_prefix=widget_key_prefix)
                        elif key == "upsert":
                            _render_upsert_stage(data, key_prefix=widget_key_prefix)
            else:
...
def _render_load_stage(data: Dict[str, Any], key_prefix: Optional[str] = None) -> None:
    ...
    if preview:
        ...
        st.text_area(
            ...
            key=f"{key_prefix}_raw" if key_prefix else None,
        )
    else:
        st.info("No text preview recorded in this trace.")


def _render_split_stage(data: Dict[str, Any], key_prefix: Optional[str] = None) -> None:
        ...
        for i, chunk in enumerate(chunks):
            ...
            with st.expander(header, expanded=(i < 2)):
                st.text_area(
                    ...
                    disabled=True,
                    label_visibility="collapsed",
                    key=f"{key_prefix}_chunk_{i}" if key_prefix else None,
                )
    else:
        st.info("No chunk text recorded. Re-run ingestion to generate new traces.")


def _render_transform_stage(data: Dict[str, Any], key_prefix: Optional[str] = None) -> None:
                ...
                if text_before or text_after:
                    ...
                    with col_before:
                        ...
                        st.text_area(
                            ...
                            disabled=True,
                            label_visibility="collapsed",
                            key=f"{key_prefix}_before_{i}" if key_prefix else None,
                        )
                    with col_after:
                        ...
                        st.text_area(
                            ...
                            disabled=True,
                            label_visibility="collapsed",
                            key=f"{key_prefix}_after_{i}" if key_prefix else None,
                        )
    else:
        st.info("No per-chunk transform data recorded. Re-run ingestion for new traces.")


def _render_embed_stage(data: Dict[str, Any], key_prefix: Optional[str] = None) -> None:
...
def _render_upsert_stage(data: Dict[str, Any], key_prefix: Optional[str] = None) -> None:
