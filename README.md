...
**Cursor 不会自动扫描并发现项目里的 MCP 服务**，必须在配置里显式声明后才会启动。

本项目已在仓库内提供 **项目级 MCP 配置**，路径为：
...
1. 用 Cursor 打开本仓库**根目录**（即包含 `.cursor` 的目录），确保工作区是单文件夹且为该仓库根目录。
2. 安装依赖：`pip install -e .` 或 `uv sync`（若使用 uv）。
3. 若 Cursor 未自动加载 MCP：打开 **Settings → MCP**，确认已识别到 `modular-rag-mcp-server`；必要时 **Reload Window**（命令面板执行 `Developer: Reload Window`）。
4. 当前工作目录会作为 MCP 进程的 cwd，因此 `config/settings.yaml`、`data/` 等路径会按项目根目录解析。

若仍不生效，可检查：当前 Cursor 使用的 Python 解释器是否为项目 venv（或已安装本项目的环境）；终端中在项目根目录执行 `python -m src.mcp_server.server` 是否能正常启动（无报错即表示命令正确）。
